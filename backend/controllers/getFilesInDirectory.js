...
const getFilesInDirectory = async (req, res, next) => {
  ...
  if (currentdirectory === "/") {
    const filesInOtherDirectories = `SELECT 
                                    directory,filename,hashvalue,last_modified,salt,iv
                                    FROM  
                                    data.files 
                                    where 
                                    username = ?
    ...
    const filesInOtherDirectories = `SELECT directory,filename,hashvalue,last_modified,salt,iv FROM files WHERE username = ? AND device = ? AND directory REGEXP ?`;
    req.headers.query = filesInOtherDirectories;
    req.headers.queryValues = [username, devicename, regex_other_files];
    await sqlExecute(req, res, next);
    req.headers.data.push(...req.headers.queryStatus);
    const filesInCurrentDirectory = `SELECT directory,filename,hashvalue,last_modified,salt,iv FROM files WHERE  username = ? AND device = ? AND directory = ?`;
